...
export default async function generatePreview(
  ...
  const fetchentitydocumentUrl = `${API_DOMAIN}/v2/accounts/me/sites/${siteId}/fetchentitydocument?entityId=${entityId}&templateId=${templateId}&deploymentId=${deploymentId}&api_key=${API_KEY}&v=${V_PARAM}`;
  console.log("fetchentitydocumentUrl", fetchentitydocumentUrl);
  // Retrieve entity document
  const entityResponse = await fetch(fetchentitydocumentUrl);
  console.log("entityResponse");
  console.log(JSON.stringify(entityResponse));
  ...
  const generateResponse = await fetch(
    `${API_DOMAIN}/v2/accounts/me/sites/${siteId}/generatepagecontent?templateId=${templateId}&servingURL=${servingURL}&deploymentId=${deploymentId}&api_key=${API_KEY}&v=${V_PARAM}`,
    {
      method: "POST",
